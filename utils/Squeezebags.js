...
class Squeezebags {
    ...
    async squeeze(message, args) {
        ...
        if (args.length === 2 && args[1] === '-tithe') {
            ...
            const confirmationFilter = (r, user) => {
                return (r.emoji.name === 'summon' || r.emoji.name === 'dispel') && user.id === message.author.id;
            };
    ...
    async summon(message, args, bag, user) {
